...
export const resolveAuthData = (event: AuthMachineEvent) => {
    if (event.type === "SIGN_IN_SUCCESS" || event.type === "REGISTRATION_VERIFICATION_SUCCESS") {
        return {
...
>(
    ({ event }) => {
        if (event.type !== "SIGN_IN_FAILURE" && event.type !== "SIGN_UP_FAILURE" && event.type !== "REGISTRATION_VERIFICATION_FAILURE") {
